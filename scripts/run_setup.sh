...
# Produces run/window.json, run/market.json, run/events.json, run/filings.json, run/pack_*.md, run/summary.md
# Optional: export SEC_UA="us-market-briefing you@example.com" (SEC asks for a contact in the User-Agent)
set -u
cd "$(dirname "$0")/.."
mkdir -p run/gather run/deep_dive
rm -f run/gather/*.md run/deep_dive/*.md run/pack_*.md run/content.json run/artifact.html run/qc_report.md run/render_report.json run/filings.json 2>/dev/null
...
echo; echo "== SEC filings (EDGAR) =="
python3 scripts/edgar_filings.py || echo "edgar_filings.py failed — the flows gatherer must sweep filings by search"
echo; echo "== packs =="
