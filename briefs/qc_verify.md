...
ex-div dates, read-through earnings), `run/filings.json` (EDGAR) and `run/window.json` (dates, weekdays, DST offset). Budget: ≤ 20 fetches/searches;
...
2. For each claim, find it in the evidence trail. If the exact figure is there with a Tier 1 URL →
   CONFIRMED without fetching. If it is there with only a Tier 2 source, re-fetch only grade-3 claims.
   If it is not there, fetch the primary or Tier-2 source and check. Do not accept a Tier-3 page as
   confirmation of a number. Spend the budget in this order: grade-3 claims → Tier-2-only figures →
   grade-2 → toplead/bottom line.
3. Check internal consistency: the same event must carry the same date/time/figure everywhere it
   ...
   consistent with the DST offset in `window.json`; nothing listed in `cut_for_cause` reappears as a
   calendar chip or timeline row.
