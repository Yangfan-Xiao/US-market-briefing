...
Search budget: 1 baseline search per name + up to 4 follow-ups for the whole group (≤ 10 total).
...
per name, and decide: does anything here plausibly clear the IMPACT FILTER? Most names on most days
produce nothing — return `none` for them (they are simply absent from KEEP; do not write "nothing to
report" lines). ETFs in your group (SPY, QQQ, IWM, USO): search the driver, not the ticker — "S&P 500
ETF flows / rebalance news" for SPY/QQQ/IWM, "crude oil ETF USO news" for USO.

**Pass 2 — follow-ups (only where Pass 1 surfaced a candidate; ≤4 for the group).**
