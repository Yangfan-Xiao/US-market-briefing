...
The continuous state variables this briefing tracks — SPY, QQQ, VIX, the 2-yr / 10-yr / 30-yr yields and the curve, WTI, the SOX semiconductor index (drives the six semis/AI names), bitcoin (drives MSTR / COIN / HOOD), Fear & Greed — normally live as Market-conditions metric cards, and a metric card can never become the day's story no matter what level it crosses. That is a structural blind spot: a 19-year high in the 30-year yield is a session driver with no 8-K, no docket and no scheduled time attached. This rule is the promotion path:
...
* A promoted variable is treated as an EVENT dated to the crossing or move. The headlines gatherer (or a deep-dive) runs a focused cause search (for SOX/BTC: the triggering company, policy or flow event) — Treasury auction results and supply calendar, corporate bond issuance (including AI-capex debt supply), fiscal/deficit headlines, inflation repricing, positioning — grounded per the SOURCE HIERARCHY.
...
1. **What is the event?** Actor, action, date/time (ET), and the document or statement it comes from. A price move alone ("NVDA −4% on AI worries") is not an event — search for the trigger; none in Tier 1/2 → "move without identified driver" → Cut for cause. (Exception: a promoted state variable, §B.)
2. **What is the mechanism?** How it changes cash flows, discount rates, share supply/demand, or the probability of a dated outcome — in one clause. No clause → colour, not a catalyst.
3. **Magnitude** — apply A.1 literally; the yardstick is past reactions to the same kind of event, not the loudness of the coverage.
4. **Already in the price?** — apply A.2 with this test: substance public before the last session's close AND the name already moved → cut. A restated fact with a new date attached is still priced in; a scheduled event's outcome never is.
5. **Reach** — apply A.3; for a watchlist name not named in the headline, state the channel (supplier, customer, competitor, index weight, rate sensitivity, input cost). "Sector sympathy" is not reach. Record it as `exposed` on the page item.
6. **Freshness / forward consequence** — apply A.4 and A.5, judged for a holder of a 7–30 DTE vertical: would they avoid opening, exit before a date, or manage a short leg?
7. **Scheduled or mechanical?** → also apply §C as hard thresholds.
8. **Source** — Tier 3 alone never grounds a page item (writing rules, source hierarchy).
