...
* **Heads-up cards are tight.** `desc` ≤55 words: mechanism + forward consequence + the key figure. Put the exposed watchlist tickers in `exposed` (ticker + channel, rendered as chips) and the dated resolver/invalidator in `resolves` — do not spend prose on either. Give every News line its `exposed` tickers too, and its `event_date`; a story more than 3 sessions old also needs `gates_date` (the dated pivot inside the hold window) or it fails Freshness. The renderer warns over budget.
...
Calendar chips: `e-bin` (dated binary) · `e-macro` (gated auctions, QRA, Fed speakers, central banks, policy deadlines, second-tier data) · `e-exdiv` · `e-flow` (index adds, rebalance flows) · `e-struct` (blackout starts/ends, lockups, offerings, splits, OpEx/witching, half-days) · `e-pos` (capital-return events) · `e-read` (non-watchlist earnings with read-through, written `MU earnings AMC → NVDA/AMD`) · `e-quiet` (only when a day has nothing).
Chips obey the same gates as page items: anything in Cut for cause gets no chip, and 2/3/5/7-yr auctions never get one (the renderer rejects them unless an assumption starts "Short-coupon auction kept:").
...
* Tier 2 — exactly these outlets: Reuters, Bloomberg, WSJ, FT, AP, CNBC, Barron's, MarketWatch, Nikkei Asia; SCMP / Caixin for China policy only. Anything not on this list is Tier 3 by default.
* Tier 3 — leads only, NEVER the sole grounding for a page item: Yahoo Finance articles, Kiplinger, Investopedia, Seeking Alpha, TipRanks, Benzinga, Motley Fool, FXLeaders, TradingKey, MarketBeat, StockTitan, ZeroHedge, HNGN, Yahoo aggregation pages, DigiTimes-via-blogs, and similar SEO/aggregator outlets. A Tier-3-only finding is re-grounded in Tier 1/2 during the run, or tagged [REPORTED — unconfirmed] and gated harder, or cut.
